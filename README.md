...
```bash
poetry run python bookkeeper/GUI.py
```

Поддерживается возможность редактирования/удаления трат посредством редактирования их в таблице.

Кроме того, при добавлении новой траты, можно открыть дополнительные поля для ручного редактирования даты и комментария.

В бюджете можно редактировать только целевой бюджет, сумма считается автоматически.

Категории так же можно добавлять, удалять или переименовывать

В случае некорректного ввода будет отображено сообщение об ошибке с пояснением, что пошло не так и как должно было быть
